...
impl Stacks {
    ...
    fn apply(&mut self, m: &Move) {
        // set the size of the target stack
        self.0[m.to - 1].extend(std::iter::repeat('*').take(m.count));

        // write backwards to the target stack
        let end = self.0[m.to - 1].len();
        let start = end - m.count;
        for i in (start..end).rev() {
            let popped = self.0[m.from - 1].pop().unwrap();
            self.0[m.to - 1][i] = popped;
        }
    }
}
